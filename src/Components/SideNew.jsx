...
const SideNew = ({ children }) => {
    ...
    return <>
        <div className='grid grid-cols-14 '>
            <aside className={sidebarOpen ? "transition-all col-start-1 col-end-3 scroll  flex flex-col w-64 h-screen px-5 py-8 overflow-y-auto bg-white border-r rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700" : " transition-all col-start-1 col-end-2 scroll  flex flex-col w-full h-screen overflow-y-auto bg-white border-r rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700"}>
                <div className={toggleOpen ? 'toggleSideBar' : 'toggleSideBar fechar'} onClick={toggleSidebar}> <img src={dot} alt="" /></div>
                {sidebarOpen ? <div className=''>
                    <div className='container'>
                        <img src={logoimg} alt="" />
                    </div>
                    <div className='sideBarInfos'> <div className='sideBarInfosUser'><img src={userImg} alt="" /><p className='dark:text-white font-bold'>{data.nome}</p></div> <div className='btn-logout text-azul' onClick={Logout}>sair</div> </div>
                    <h2 className='dark:text-white font-bold'>Navegação</h2>
                    <div className='navSideBar'>
                        <NavLink className='nav-list' to='/dashboard' exact><FontAwesomeIcon icon={faChartPie} /> Dashboard</NavLink>
                        <NavLink className='nav-list' to='/usuarios' exact><FontAwesomeIcon icon={faUsers} /> Usuários</NavLink>
                        <NavLink className='nav-list' to='/tarefas' exact><FontAwesomeIcon icon={faListCheck} /> Tarefas</NavLink>
                    </div>

                    <h2 className='dark:text-white font-bold'>Pastas</h2>
                    <div className='flex gap-5 flex-col'>
                        {options.map((option) => (
                            <NavLink className='nav-list' to={{ pathname: `/pasta/${option.id_pasta}`, state: { pasta: option.id_pasta } }} exact>{option.pasta}</NavLink>
                        ))}
                    ...
                    <aside class="flex flex-col items-center w-16 h-screen py-8 overflow-y-auto bg-white border-r rtl:border-l rtl:border-r-0 dark:bg-gray-900 dark:border-gray-700">
                        <nav class="flex flex-col flex-1 space-y-6">
                            <a href="#">
                                <img class="w-auto h-6 " src="https://merakiui.com/images/logo.svg" alt=""></img>
                            </a>
            ...
            <main className={sidebarOpen ? 'col-start-3 col-end-14  ' : 'col-start-2 col-end-14 w-100'} >
                {children}
            </main>
        </div>
    </>
