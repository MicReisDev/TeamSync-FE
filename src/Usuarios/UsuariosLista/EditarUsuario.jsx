...
const EditarUsuario = ({ usuario, openModalEditar, setOpenModalEditar, setUpdateComponent, updateComponent }) => {
    const id = usuario.id_usuario ? usuario.id_usuario : 0
    ...
    const Editar = async (e) => {
        ...
        const dadosParaEnviar = { ...dadosDoFormulario, pastas: selectedPastas };
        try {

            const response = await fetch(`http://localhost:33331/editar-usuario/${id}`, {
                method: 'put',
                headers: {
            ...
            if (data.error) {
                ...
                setDadosDoFormulario({
                    ...
                    senha: '',
                    confirmSenha: '',
                    role: usuario.roles ? usuario.roles[0].id_role : '',
    ...
    useEffect(() => {
        ...
        setSelectedPastas(usuario.pastas || []);

    }, [usuario])
    return <>

        <div className='flex flex-col scroll px-5 py-2 bg-white rounded-3xl'>
            <div className='flex ' onClick={() => { setOpenModalEditar(false) }}>
            ...
            <div className='max-w-lg mx-auto flex flex-col justify-center items-center'>
                {userLoading ? <div role="status">
                    ...
                    <img
                        ...
                        src={dadosDoFormulario.avatar ? dadosDoFormulario.avatar : userIMG}
            ...
            <form className='flex flex-col' onSubmit={Editar}>
                ...
                <CheckBoxComponent
                    selectedPastas={selectedPastas}
                    setSelectedPastas={setSelectedPastas}
                    allPastas={allPastas}
                />
                <EditRoleSelect
                    setRole={(role) => setDadosDoFormulario({ ...dadosDoFormulario, role })}
                    dados={dadosDoFormulario}
                />
                <ButtonDefault nome={'Editar Usuário'}></ButtonDefault>
            </form>
        </div>
